    ...
    */
    messageWrapper(message) {
        if (this.messageHandler) {
            ...
            if (user) {
                message.user = this.rtm.dataStore.getUserById(user);
            }
            ...
            if (bot_id) {
                message.bot = this.rtm.dataStore.getBotById(bot_id);
            }
            if (channel) {
                message.channel = this.rtm.dataStore.getChannelGroupOrDMById(channel);
            }
        ...
        switch (type) {
            ...
            case "group":
                return this.web.groups.setTopic(id, topic, (err, res) => {
                    if (err || !res.ok) {
                        return this.robot.logger.debug("Cannot set topic in MPIM");
                    }
                });
    ...
    */
    ...
    loadUsers(callback) {
        ...
        var pageLoaded = (error, results) => {
            ...
            for (let member of Array.from(results.members)) {
